...
void smf_sess_create_cp_up_data_forwarding(smf_sess_t *sess)
{
    ...
    /* QER */
    up2cp_qer = ogs_pfcp_qer_add(&sess->pfcp);
    ogs_assert(up2cp_qer);
    sess->up2cp_qer = up2cp_qer;
    
    ogs_pfcp_pdr_associate_qer(up2cp_pdr, up2cp_qer);
    ogs_pfcp_pdr_associate_qer(cp2up_pdr, up2cp_qer);
    
    up2cp_qer->qfi = *(qos_flow->qfi_node);
}
...
void smf_sess_delete_cp_up_data_forwarding(smf_sess_t *sess)
{
    ...
    if (sess->up2cp_far)
     ...
     if (sess->up2cp_qer)
        ogs_pfcp_qer_remove(sess->up2cp_qer);
}
...
smf_bearer_t *smf_qos_flow_find_by_qfi(smf_sess_t *sess, uint8_t qfi)
{
    ...
    ogs_assert(sess);
    ogs_list_for_each(&sess->bearer_list, qos_flow) {
//        ogs_error("qos_flow->qfi is %d",qos_flow->qfi);
        if (qos_flow->qfi == qfi)
            return qos_flow;
    }
