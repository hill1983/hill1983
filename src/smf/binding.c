...
void smf_qos_flow_binding(smf_sess_t *sess)
{
    ...
    ogs_assert(sess);
    //smf_bearer_t **qos_tmp = {NULL};
    smf_bearer_t *qos_tmp[4]={NULL,NULL,NULL,NULL};
 ...
 //           memcpy(&qos_tmp[i],qos_flow,sizeof(smf_bearer_t));
 ...
 */
 ...
 //           ogs_error("qos_flow_created is %d",qos_flow_created);
            qos_tmp[i]=qos_flow;
            smf_bearer_tft_update(qos_flow);
            smf_bearer_qos_update(qos_flow);
        }
    }
 //           memcpy(*qos_tmp,qos_flow,sizeof(smf_bearer_t));
 //           ogs_error("qos_tmp[%d] id is %p",i,qos_tmp[i]);
 //           ogs_error("qos_tmp[%d]->sess id is %d",i,(&qos_tmp[i])->sess->index);
 //           ogs_error("current qos_flow address is %p",qos_flow+i);
            if (qos_flow_created == true) {
//                smf_bearer_tft_update(*qos_tmp);
                smf_bearer_qos_update(*qos_tmp);
/*
                ogs_assert(OGS_OK ==
                    smf_5gc_pfcp_send_qos_flow_modification_request(
                        ...
                        */
                ogs_assert(OGS_OK ==
                    smf_5gc_pfcp_send_qos_flow_modification_request_video(
                        (smf_bearer_t **)qos_tmp, NULL,
                        OGS_PFCP_MODIFY_NETWORK_REQUESTED|
                        OGS_PFCP_MODIFY_CREATE,i));
                return;
...
/*
                ...
                if (ogs_list_count(qos_tmp->pf_to_add_list) > 0) {
                ...
                ogs_assert(OGS_OK ==
                    smf_5gc_pfcp_send_qos_flow_modification_request_video(
                        (smf_bearer_t **)qos_tmp, NULL, pfcp_flags,i));
                return;
            }
*/
 ...
 for (i = 0; i < sess->policy.num_of_pcc_rule; i++) {
        ...
        if (pcc_rule->id == NULL) {
            ...
            if (pcc_rule->type == OGS_PCC_RULE_TYPE_REMOVE) {
            qos_flow = smf_qos_flow_find_by_pcc_rule_id(sess, pcc_rule->id);
            ...
            ogs_error("Remove_number is %d",i);
                ogs_assert(OGS_OK ==
                    smf_5gc_pfcp_send_qos_flow_modification_request_video(
                        (smf_bearer_t **)qos_tmp, NULL,
                        OGS_PFCP_MODIFY_NETWORK_REQUESTED|OGS_PFCP_MODIFY_REMOVE,i));
*/
        j=0;
		for (i = 0; i < sess->policy.num_of_pcc_rule; i++) {
        smf_bearer_t *qos_flow = NULL;
...
//     		ogs_error("Num of qos_flow to Remove is %d",j);
            ogs_assert(OGS_OK ==
            smf_5gc_pfcp_send_qos_flow_modification_request_video(
                        (smf_bearer_t **)qos_tmp, NULL,
                        pfcp_flags,j));
