...
ogs_pkbuf_t *ngap_build_qos_flow_resource_modify_request_transfer_video(
        smf_bearer_t *qos_flow, smf_bearer_t *qos_flow_video, bool qos_presence)
{
    ogs_debug("---Enter func---");
    NGAP_PDUSessionResourceModifyRequestTransfer_t message;
    ...
    *qosFlowIdentifier = qos_flow_video->qfi;

    nonDynamic5QI->fiveQI = qos_flow_video->qos.index;

    allocationAndRetentionPriority->priorityLevelARP =
        qos_flow_video->qos.arp.priority_level;
    if (qos_flow_video->qos.arp.pre_emption_capability ==
            OGS_5GC_PRE_EMPTION_ENABLED)
        allocationAndRetentionPriority->pre_emptionCapability =
            NGAP_Pre_emptionCapability_may_trigger_pre_emption;
    if (qos_flow_video->qos.arp.pre_emption_vulnerability ==
            OGS_5GC_PRE_EMPTION_ENABLED)
        allocationAndRetentionPriority->pre_emptionVulnerability =
            NGAP_Pre_emptionVulnerability_pre_emptable;

    if (qos_presence == true &&
        (qos_flow_video->qos.mbr.downlink || qos_flow_video->qos.mbr.uplink ||
         qos_flow_video->qos.gbr.downlink || qos_flow_video->qos.gbr.uplink)) {
        ogs_assert(qos_flow_video->qos.mbr.downlink);
        ogs_assert(qos_flow_video->qos.mbr.uplink);
        ogs_assert(qos_flow_video->qos.gbr.downlink);
        ogs_assert(qos_flow_video->qos.gbr.uplink);
        ...
        asn_uint642INTEGER(&gBR_QosInformation->
                guaranteedFlowBitRateDL, qos_flow_video->qos.gbr.downlink);
        asn_uint642INTEGER(&gBR_QosInformation->
                guaranteedFlowBitRateUL, qos_flow_video->qos.gbr.uplink);
    }
    return ogs_asn_encode(
            &asn_DEF_NGAP_PDUSessionResourceModifyRequestTransfer, &message);
}
...
ogs_pkbuf_t *ngap_build_qos_flow_resource_release_request_transfer_video(
        smf_bearer_t *qos_flow, smf_bearer_t *qos_flow_video, NGAP_Cause_PR group, long cause)
{
    ogs_debug("---Enter func---");
    NGAP_PDUSessionResourceModifyRequestTransfer_t message;
