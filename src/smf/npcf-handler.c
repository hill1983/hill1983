...
#define MAX_TRIGGER_ID 128
    ...
    ul_pdr->ue_ip_addr.sd = OGS_PFCP_UE_IP_SRC;
    ...
    up2cp_pdr->flow_description[up2cp_pdr->num_of_flow++] =
       ...
       // (char *)"permit out 58 from any to ff02::2/128";
    ogs_assert(OGS_OK ==
        ...
        smf_sess_t *sess, ogs_sbi_stream_t *stream, ogs_sbi_message_t *recvmsg)
{
    ...
    OpenAPI_sm_policy_decision_t *SmPolicyDecision = NULL;
    ogs_debug("smf_npcf_smpolicycontrol_handle_update_notify");
    ogs_assert(sess);
    ogs_assert(stream);
    smf_ue = sess->smf_ue;
    ogs_assert(smf_ue);
    ...
    /* Update authorized PCC rule & QoS */
    update_authorized_pcc_rule_and_qos(sess, SmPolicyDecision);
//    ogs_error("sess->policy.num_of_pcc_rule is :%d",sess->policy.num_of_pcc_rule);
