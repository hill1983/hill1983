        ...
        ogs_sbi_stream_t *stream, uint64_t flags)
{
    ...
    ogs_pfcp_xact_t *xact = NULL;
    smf_sess_t *sess = NULL;
    smf_bearer_t *qos_flow_tmp;

    ogs_assert(qos_flow);
    sess = qos_flow->sess;
    ogs_assert(sess);
    ogs_error("qos_flow address is %p",qos_flow);
    ogs_error("qos_flow->dl_pdr id is %d",qos_flow->dl_pdr->id);
    ogs_error("qos_flow->sess id is %d",qos_flow->sess->index);
    ogs_error("qos_flow size is %ld",sizeof(smf_bearer_t));
    ...
    ogs_expect_or_return_val(n4buf, OGS_ERROR);
//    memcpy(qos_flow_tmp,qos_flow,sizeof(smf_bearer_t));
    qos_flow_tmp=qos_flow;
    xact = ogs_pfcp_xact_local_create(
            sess->pfcp_node, &h, n4buf, qos_flow_5gc_timeout, qos_flow_tmp);
    ogs_expect_or_return_val(xact, OGS_ERROR);

    xact->assoc_stream = stream;
    xact->modify_flags = flags;
    ogs_error("xact id is %d",xact->xid);
    ogs_error("xact-data address is %p",xact->data);
    rv = ogs_pfcp_xact_commit(xact);
    ogs_expect(rv == OGS_OK);
...
int smf_5gc_pfcp_send_qos_flow_modification_request_video(smf_bearer_t **qos_flow,
        ogs_sbi_stream_t *stream, uint64_t flags ,int num)
{
    ogs_debug("---Enter func---");
    int rv;
    ogs_pkbuf_t *n4buf = NULL;
...
//    smf_bearer_t *qos_flow_tmp;
    ...
    int i;
    i=0;
    ogs_assert(qos_flow[i]);
    sess = qos_flow[i]->sess;
    ogs_assert(sess);
/*    ogs_error("qos_flow address is %p",qos_flow);
    ogs_error("qos_flow->dl_pdr id is %d",qos_flow[i]->dl_pdr->id);
    ogs_error("qos_flow->sess id is %d",qos_flow[i]->sess->index);
    ogs_error("qos_flow size is %ld",sizeof(smf_bearer_t));
*/
    memset(&h, 0, sizeof(ogs_pfcp_header_t));
    h.type = OGS_PFCP_SESSION_MODIFICATION_REQUEST_TYPE;
...
//    n4buf = smf_n4_build_qos_flow_modification_request(h.type, qos_flow[i], flags);
    n4buf = smf_n4_build_qos_flow_modification_request_video(h.type, qos_flow, flags, num);
    ogs_expect_or_return_val(n4buf, OGS_ERROR);
//    memcpy(qos_flow_tmp,qos_flow,sizeof(smf_bearer_t));
//    qos_flow_tmp=qos_flow;
 /*   for(i=0;i<num;i++){
                ogs_error("qos_flow[%d] id is %d",i,qos_flow[i]->ul_pdr->id);
                ogs_error("teid of PDR is %d",qos_flow[i]->ul_pdr->f_teid.teid);
                ogs_error("ch of PDR is %d",qos_flow[i]->ul_pdr->f_teid.ch);
                ogs_error("chid of PDR is %d",qos_flow[i]->ul_pdr->f_teid.chid);
                ogs_error("choose_id of PDR is %d",qos_flow[i]->ul_pdr->f_teid.choose_id);
    }*/
    xact = ogs_pfcp_xact_local_create_video(
            sess->pfcp_node, &h, n4buf, qos_flow_5gc_timeout, qos_flow[0], qos_flow[1]);
    ogs_expect_or_return_val(xact, OGS_ERROR);

    xact->assoc_stream = stream;
    xact->modify_flags = flags;
//    ogs_error("xact id is %d",xact->xid);
//    ogs_error("xact-qos_flow address is %p",xact->data);
    rv = ogs_pfcp_xact_commit(xact);
    ogs_expect(rv == OGS_OK);
