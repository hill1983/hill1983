        ...
        ogs_pfcp_session_modification_response_t *rsp)
{
    ...
    smf_bearer_t *qos_flow_video = NULL;
 ...
 //       qos_flow = xact->data;
     ...
     */
        ...
        if (flags & OGS_PFCP_MODIFY_INDIRECT) {
            ...
            if(qos_flow_video)
            {
                param.n1smbuf = gsm_build_qos_flow_modification_command_video(
                    qos_flow,qos_flow_video,
            ...
            {
                param.n2smbuf =
                ngap_build_qos_flow_resource_release_request_transfer_video(
                        qos_flow,qos_flow_video,
                        NGAP_Cause_PR_nas, NGAP_CauseNas_normal_release);
            }else
            {
            ...
            if(qos_flow_video)
            {
                smf_bearer_remove(qos_flow_video);
            }
        ...
        if (flags & OGS_PFCP_MODIFY_INDIRECT) {
            ...
            {
            param.n2smbuf =
                ngap_build_qos_flow_resource_modify_request_transfer_video(
                        qos_flow, qos_flow_video,true);
            }else{
            param.n2smbuf =
