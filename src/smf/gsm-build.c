...
ogs_pkbuf_t *gsm_build_qos_flow_modification_command_video(
        smf_bearer_t *qos_flow, smf_bearer_t *qos_flow_video, uint8_t pti,
        uint8_t qos_rule_code, uint8_t qos_flow_description_code)
{
    ogs_debug("gsm_build_qos_flow_modification_command_video");
    ...
    if (qos_rule_code) {
        ...
        qos_rule[1].identifier = qos_flow_video->qfi; /* Use QFI in Open5GS */
        qos_rule[1].code = qos_rule_code;

        if (qos_rule_code != OGS_NAS_QOS_CODE_DELETE_EXISTING_QOS_RULE &&
            qos_rule_code != OGS_NAS_QOS_CODE_MODIFY_EXISTING_QOS_RULE_WITHOUT_MODIFYING_PACKET_FILTERS)
            encode_qos_rule_packet_filter(&qos_rule[1], qos_flow_video);
        ...
        if (qos_rule_code != OGS_NAS_QOS_CODE_DELETE_EXISTING_QOS_RULE &&
            ...
            ogs_assert(dl_pdr->precedence > 0 && dl_pdr->precedence < 255);
            /* Use PCC Rule Precedence */
            qos_rule[1].precedence = qos_flow_video->dl_pdr->precedence;

            qos_rule[1].flow.segregation = 0;
            qos_rule[1].flow.identifier = qos_flow_video->qfi;
        }

        rv = ogs_nas_build_qos_rules(authorized_qos_rules, qos_rule, 2);
        ogs_expect_or_return_val(rv == OGS_OK, NULL);
        ogs_expect_or_return_val(authorized_qos_rules->length, NULL);
    ...
    if (qos_flow_description_code) {
        ...
        qos_flow_description[1].identifier = qos_flow_video->qfi;
        qos_flow_description[1].code = qos_flow_description_code;

        num_of_param = 0;

        if (qos_flow_description_code != OGS_NAS_DELETE_NEW_QOS_FLOW_DESCRIPTION) {
            qos_flow_description[1].E_bit = 1;

            qos_flow_description[1].param[num_of_param].identifier =
                OGS_NAX_QOS_FLOW_PARAMETER_ID_5QI;
            qos_flow_description[1].param[num_of_param].len =
                sizeof(qos_flow_description[0].param[num_of_param].qos_index);
            qos_flow_description[1].param[num_of_param].qos_index =
                qos_flow_video->qos.index;
            num_of_param++;

            if (qos_flow_video->qos.gbr.uplink) {
                qos_flow_description[1].param[num_of_param].identifier =
                    OGS_NAX_QOS_FLOW_PARAMETER_ID_GFBR_UPLINK;
                qos_flow_description[1].param[num_of_param].len =
                    sizeof(qos_flow_description[1].param[num_of_param].br);
                ogs_nas_bitrate_from_uint64(
                        &qos_flow_description[1].param[num_of_param].br,
                        qos_flow_video->qos.gbr.uplink);
                num_of_param++;
            }
            if (qos_flow_video->qos.gbr.downlink) {
                qos_flow_description[1].param[num_of_param].identifier =
                    OGS_NAX_QOS_FLOW_PARAMETER_ID_GFBR_DOWNLINK;
                qos_flow_description[1].param[num_of_param].len =
                    sizeof(qos_flow_description[1].param[num_of_param].br);
                ogs_nas_bitrate_from_uint64(
                        &qos_flow_description[1].param[num_of_param].br,
                        qos_flow_video->qos.gbr.downlink);
                num_of_param++;
            }
            if (qos_flow_video->qos.mbr.uplink) {
                qos_flow_description[1].param[num_of_param].identifier =
                    OGS_NAX_QOS_FLOW_PARAMETER_ID_MFBR_UPLINK;
                qos_flow_description[1].param[num_of_param].len =
                    sizeof(qos_flow_description[1].param[num_of_param].br);
                ogs_nas_bitrate_from_uint64(
                        &qos_flow_description[1].param[num_of_param].br,
                        qos_flow_video->qos.mbr.uplink);
                num_of_param++;
            }
            if (qos_flow_video->qos.mbr.downlink) {
                qos_flow_description[1].param[num_of_param].identifier =
                    OGS_NAX_QOS_FLOW_PARAMETER_ID_MFBR_DOWNLINK;
                qos_flow_description[1].param[num_of_param].len =
                    sizeof(qos_flow_description[1].param[num_of_param].br);
                ogs_nas_bitrate_from_uint64(
                        &qos_flow_description[1].param[num_of_param].br,
                        qos_flow_video->qos.mbr.downlink);
                num_of_param++;
            }
        }

        qos_flow_description[1].num_of_parameter = num_of_param;

        rv = ogs_nas_build_qos_flow_descriptions(
                authorized_qos_flow_descriptions, qos_flow_description, 2);
        ogs_expect_or_return_val(rv == OGS_OK, NULL);
        ogs_expect_or_return_val(
                authorized_qos_flow_descriptions->length, NULL);
